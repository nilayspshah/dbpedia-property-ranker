...
def get_resource_type(results):
    '''
    Computes the search_term type of the search_term. 
    Example:For search_term =' Iron_Maiden', returns resource_type = 'http://dbpedia.org/ontology/Organisation'
    '''
    resource_type = None
...
def google_autocomplete_ranker(search_term, prop):
    '''
    Function which counts the number of characters from prop name needed to be pressed so that the prop is 
    suggested by Google auto-complete pertaining to the current search_term.
    ...
    while (google_keypresses <= len(prop) and google_keypresses <= 4):
        ...
        request_string = request_url + search_term + '_' + \
...
def handle_is_of_relations(search_term, resource_type, total_pages):

    sparql = SPARQLWrapper("http://dbpedia.org/sparql")

    query1 = """
    select  ?prop ?value where {
    ?value ?prop <http://dbpedia.org/resource/""" + search_term + """>  }
    """
    ...
    for result in results["results"]["bindings"]:
        ...
        if "xml:lang" not in result["value"] or 'en' in result["value"]["xml:lang"]:
            ...
            prop_info['blacklisted'] = blacklisted(cleaned_property_label)
            '''
            ...
            google_autosuggest = google_autocomplete_ranker(
                search_term, cleaned_property_label)
...
def count_freq(resource_type, prop):
    '''
    Function which counts how many times has the property appeared w.r.t. the search_term type.
    '''
    freq = 0
    sparql = SPARQLWrapper("http://dbpedia.org/sparql")
...
def total_pages_for_type(resource_type):
    '''
    Function which returns the total number of resources belonging to given search_term type.
    '''
    freq = 0
    sparql = SPARQLWrapper("http://dbpedia.org/sparql")
...
def start(search_term):
    ...
    where {
    <http://dbpedia.org/resource/""" + search_term + """> ?prop ?value }
      """
    ...
    #handle_is_of_relations(search_term, resource_type,  total_pages)

    for prop, count in prop_val_count.iteritems():
        ans_dict[prop]['total_values'] = (1.0 - 1.0 / count)
        score = raw_input("Enter score for: " + prop + " (from 0-4) \n")
        ans_dict[prop]['score'] = score


res = "Methane"
start(res)
open_file = res + "_train.txt"
